...
export default function Integrations() {
  return (
    <section className="relative py-24 md:py-28 overflow-hidden bg-transparent">
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {tools.map((t) => (
            <div
              key={t}
              className="flex items-center justify-center px-4 py-4 rounded-xl text-sm font-semibold text-slate-400 transition-colors duration-200 hover:text-slate-100"
              style={{ background: "transparent", border: "1px solid rgba(11,18,32,0.06)" }}
            >
              {t}
            </div>
          ))}
        </div>
        <p className="text-center text-sm text-slate-500 mt-6">En elk ander systeem met een koppeling. Staat jouw tool er niet bij? We bouwen de koppeling.</p>
      </div>
    </section>
  );
}
